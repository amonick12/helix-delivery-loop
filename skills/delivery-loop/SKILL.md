...
- **Epic-final TestFlight gate** — when an epic's LAST sub-card has both `code-review-approved` and `visual-qa-approved`, postagent EC-10 fires `scripts/notify-epic-testflight.sh`: builds TestFlight, gathers all sub-card screenshots, emails `amonick12@gmail.com` (override via `EPIC_NOTIFY_TO`), and adds `epic-testflight-pending` to the last PR. Releaser's "Epic TestFlight gate" refuses to merge until user adds `epic-final-approved` (or `user-approved`) after testing on device.
