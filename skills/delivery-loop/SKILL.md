...
- **Epic-final TestFlight gate** — when an epic's LAST sub-card has both `code-review-approved` and `visual-qa-approved`, postagent EC-10 fires `scripts/notify-epic-testflight.sh`: builds TestFlight, gathers all sub-card screenshots, **queues an email** at `/tmp/helix-epic-emails-pending/epic-<N>.json`, and adds `epic-testflight-pending` to the last PR. The `/delivery-loop` orchestrator drains the queue at every dispatch via the **Gmail MCP** (`mcp__claude_ai_Gmail__send_message`) — shell scripts can't invoke MCP tools directly, so the queue-and-drain pattern. Releaser's "Epic TestFlight gate" refuses to merge until user adds `epic-final-approved` (or `user-approved`) after testing on device.
...
- **TestFlight is on-demand** — user comments "deploy" on a PR, Releaser uploads (not a merge gate). Epic-final TestFlight is automatic, see above.
