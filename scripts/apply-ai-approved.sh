#!/bin/bash
# apply-ai-approved.sh — Single enforcement script for ai-approved label.
#
# Usage:
#   ./apply-ai-approved.sh --pr 42 --card 137
#
# Internally:
#   1. Calls update-pr-checklist.sh → checks all_checked
#   2. If HasUIChanges=Yes: calls verify-evidence.sh → checks eligible
#   3. If both pass: applies "ai-approved" label
...
Steps:
  1. Calls update-pr-checklist.sh → checks all_checked
  2. If HasUIChanges=Yes: calls verify-evidence.sh → checks eligible
  3. If both pass: applies "ai-approved" label via gh
  4. If either fails: prints what's missing, does NOT apply label
...
if [[ "$ALL_CHECKED" == "true" && "$EVIDENCE_ELIGIBLE" == "true" ]]; then
  ...
  if [[ "$DRY_RUN" == "1" ]]; then
    log_info "[DRY_RUN] Would apply ai-approved label to PR #$PR_NUMBER"
  else
    # Idempotency: check if label already applied
    ALREADY_APPROVED=$(gh pr view "$PR_NUMBER" --repo "$REPO" --json labels --jq 'any(.labels[]; .name == "ai-approved")' 2>/dev/null || echo "false")
    if [[ "$ALREADY_APPROVED" == "true" ]]; then
      log_info "ai-approved label already present on PR #$PR_NUMBER"
    else
      gh pr edit "$PR_NUMBER" --repo "$REPO" --add-label "ai-approved"
      log_info "Applied ai-approved label to PR #$PR_NUMBER"
    fi
    # Clean up transitional labels that should not coexist with approval
    gh pr edit "$PR_NUMBER" --repo "$REPO" --remove-label "awaiting-visual-qa" 2>/dev/null || true
    gh pr edit "$PR_NUMBER" --repo "$REPO" --remove-label "awaiting-code-review" 2>/dev/null || true
    # Move card to In Review when ai-approved is applied
  ...
  log_warn "NOT applying ai-approved: $REASON"
fi
